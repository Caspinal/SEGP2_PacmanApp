...
public class PacManV2_Experimental : MonoBehaviour {
	
	// Use this for initialization
	

	// Connors experimental change
	public float smoothTime = 0.25F;
	private Vector3 velocity = new Vector3(0f,0f,1f);
	public Vector3 targetpos;
	float MoveDelayTimer = 2.0f;
	bool trigger2 = true;
	...
	// new code form connor
	//public bool respawn = false;

	int pacmanX;						// stores Pacmans X co-ordinate 
	int pacmanY;
	bool globalRespawn = false;
	//bool spawntrigger = true;
	int SpawnPosX;// stores Pacmans Y co-ordinate
	int SpawnPosY;
	...
	IEnumerator PressUpKey() {						 // this method is executed in the first frame and continues until complete
		...
		//if (trigger2 == true) {
			...
			//trigger2 = false;
		//}// when the user presses up, the StartMoving() method is launched
	}
...
//		yield return new WaitForSeconds(MoveDelayTimer);
//		counter = 0;
//		//moving = true;
//		StartCoroutine (StartMoving ());
//		MoveDelayTimer = 2f;
//		Debug.Log("all should be moving");
//		bool spawntrigger = true;
//	}
		...
		while (moving) {				 // while we are moving, this is always true at this moment
			...
			case 0:															// if pacman is facing up											
				if(DH.StoredRefCoordinates[pacmanX, pacmanY+1] != 'w') {	// check if there is a wall infront of us
					...
					//connor
					targetpos  = new Vector3(pacmanX, 0, pacmanY);
					//targetpos = transform.TransformPoint(new Vector3(pacmanX, 0, pacmanY));
...
//					transform.position = new Vector3(transform.position.x,transform.position.y,(transform.position.z + 0.25f));
				...
				yield return new WaitForSeconds(0.025f);						// wait 0.16 seconds before we move another space //changed to 0.025 by connor
...
//			break;
	...
	void Update() {
		if (trigger == true) {  					// enter if first frame as trigger is true
			...
			origaRotation = transform.rotation;// start the coroutine to move pacman in the direction it is facing
		}

		if (DH.PacmanX != pacmanX) {
			DH.PacmanX = pacmanX;
		}

		if (DH.PacmanY != pacmanY) {
			DH.PacmanY = pacmanY;
		}
		...
		if (DH.globalRespawn == true) {
			//if (spawntrigger == false){
					...
					DH.PacmanX = pacmanX;
				transform.position = new Vector3(pacmanX, 0, pacmanY);
				transform.rotation = origaRotation;
						//respawn();
